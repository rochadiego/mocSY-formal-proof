...
ctrlDevLogic _ Abst m cv
    ...
    where n = 4
ctrlDevLogic (fInner,gInner,m') (Prst r) m (a,b,c)
    | m > 0 = ((a,b,c), m-1, list4tuple (replicate n Abst))
    | r == a = ((d,b,c), m', list4tuple (func n (d-1) (Prst (fInner,gInner,m')) Abst))
    | r == b = ((a,d,c), m', list4tuple (func n (d-1) (Prst (fInner,gInner,m')) Abst))
    | r == c = ((a,b,d), m', list4tuple (func n (d-1) (Prst (fInner,gInner,m')) Abst))
